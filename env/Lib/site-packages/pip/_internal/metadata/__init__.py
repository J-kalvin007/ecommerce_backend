...
def _should_use_importlib_metadata() -> bool:
    ...
    * If environment variable ``_PIP_USE_IMPORTLIB_METADATA`` is set, it
      dictates whether ``importlib.metadata`` is used, for Python <3.12.
    * On Python 3.11, 3.12 and 3.13, Python distributors can patch
    ...
    On Python 3.12+, the ``pkg_resources`` backend cannot be used.
    """
    if sys.version_info >= (3, 12):
        # On Python >=3.12 we only support importlib.metadata.
        return True
    with contextlib.suppress(KeyError, ValueError):
        # On Python <3.12, if the environment variable is set, we obey what it says.
